...
export default function Navbar() {
  ...
  const logoText: React.CSSProperties = {
    fontWeight: 800, fontSize: 16, color: "#14532d", letterSpacing: "-0.3px",
  };
    ...
    <header style={nav}>
      <div style={inner}>
        ...
        <button onClick={() => setOpen(!open)} className="md:hidden"
          style={{ background: "none", border: "1px solid #e2e8f0", borderRadius: 7, padding: 7, cursor: "pointer", color: "#334155" }}>
          {open ? <X size={18} /> : <Menu size={18} />}
        </button>
      </div>

      {open && (
        <div style={{ background: "#fff", borderTop: "1px solid #dcfce7", padding: "12px 20px 16px" }}>
          {links.map(l => (
            <a key={l.label} href={l.href} onClick={() => setOpen(false)}
              style={{ display: "block", padding: "8px 0", borderBottom: "1px solid #f0fdf4", fontSize: 15, fontWeight: 600, color: "#1e293b", textDecoration: "none" }}>{l.label}</a>
          ))}
